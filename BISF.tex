...
\end{description}
	...
	questo perchè alcuni scambi conclusi in chiusura potrebbero alterare il valore in chiusura %%%Not important
\newpage
...
\end{center}
i rendimenti verranno analizzati più tardi %% Reference pls
\newpage
...
\subsection{Strategie di Timing}
	Esistono diversi metodi per sviluppare delle strategie, queste includono l'utilizzo di modelli per prevedere i rendimenti %ref ai rendimenti chapt1
...
\end{center}
	questo ci dice che r è il rendimento %%%% SEE ref:returns
...
\end{center}
	Il rendimento lordo di un mese è l'interpretazione del valore di {\EUR{1.00}} nell'asset per un mese.\newline
...
\subsection{Linearity Assumption}
	Assumiamo che le variabili $x_{t+h} xt,...,xt-p$ abbiamo una distribuzione normale, dove il processo ${X_t}$ è la Gaussiana, allora

\begin{center}
	$E(X_{t+h}|X_t,X_{t-1}...X_{t-p})=\alpha_0 X_t+ \alpha_1 X_{t-1}...+\alpha_p X_{t-p}$
\end{center}
con $\alpha$ valori reali.

	Secondo questa, il problema di costruire la miglior predizione per il processo Gaussiano è un tipo di \emph{regressione lineare}

	Per ogni altro processo, non distribuito normalmente, desideriamo progettare un predittore come combinazione lineare del suo passato, anche se questo non coincide con i valori attesi data la sua "history"
